...
function Dialog({open, initialNote, closeDialog, postNote: postNoteState, patchNote: patchNoteState}) {
    ...
    const patchNote = async (_id, title, content) => {
        try {
          const response = await fetch(`http://localhost:4000/patchNote/${_id}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          patchNoteState(updatedNote); // Update the frontend state with the updated note
          setStatus('Note updated successfully.'); // Assuming setStatus updates the dialog status message
        } catch (error) {
