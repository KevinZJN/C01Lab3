...
function App() {
  ...
  const deleteNote = async (noteId) => {
    try {
      ...
      deleteNoteState(noteId); // Update the frontend state
    } catch (error) {
  ...
  const deleteAllNotes = async () => {
    try {
      ...
      console.log(result.response); // Log the response from the server
      deleteAllNotesState(); // Clear the frontend state
    } catch (error) {
  ...
  const deleteAllNotesState = () => {
    setNotes([]); // Clear the notes state array
  };

  const patchNoteState = (updatedNote) => {
